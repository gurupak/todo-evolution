...
Validates Dapr components against constitution security and governance rules.
"""
...
class DaprValidator:
    ...
    def _validate_database_component(self, spec: Dict[str, Any], filepath: str):
        ...
        ssl_mode = None
        for item in metadata:
            if isinstance(item, dict) and item.get("name") == "sslmode":
                ssl_mode = item.get("value", "")

        if not ssl_mode or ssl_mode != "require":
            self.violations.append(
                ValidationResult(
                    "DB-001",
                    "CRITICAL",
                    "Database component missing sslmode=require",
                    f"{filepath}:spec.metadata",
                    "Add metadata item: name: sslmode, value: require",
                )
            )
    ...
    def _validate_kafka_component(self, spec: Dict[str, Any], filepath: str):
        ...
        for item in metadata:
            if isinstance(item, dict):
                name = item.get("name", "").lower()
                if "sasl" in name or "auth" in name or "username" in name:
                    has_auth = True
                    break
...
def print_report(passed: bool, violations: List[ValidationResult], filepath: str):
    ...
    print(f"\n{'=' * 80}")
    print(f"Dapr Component Violation Report: {filepath}")
    print(f"{'=' * 80}\n")
    ...
    if not violations:
        print("✓ No violations found - component complies with constitution rules\n")
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python validate_dapr.py <component-file>")
        sys.exit(1)
